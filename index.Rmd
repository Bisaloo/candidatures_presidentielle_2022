...
```{r}
d <- parrainages_2022 %>%
  mutate(Candidat = fct_lump_min(Candidat, 20)) %>%
...
d %>%
  ggplot(aes(y = fct_rev(Candidat), fill = fct_rev(`Date de publication`))) +
    ...
    labs(
      ...
      caption = "Pour des questions de lisibilité, seul·le·s candidat·e·s avec plus de 20 parrainages sont présent·e·s sur ce graphique.\nLes candidat·e·s qualifié·e·s sont indiqué·e·s en gras.\n\nSources:\n- données: https://presidentielle2022.conseil-constitutionnel.fr\n- images: franceinfo\n\nCréé par @grusonh."
    ) +
    annotate(
